...
class AccountsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        accountStore?.requestAccessToAccountsWithType(accountType, options: nil, completion: {granted,
            error in
                if(granted)
                {
                   self.twitterAccounts = self.accountStore!.accountsWithAccountType(accountType)
                    ...
                    {
                        var noAccountsAlert : UIAlertController = UIAlertController(title: "No Accounts Found", message: "No Twitter Accounts Were Found", preferredStyle: UIAlertControllerStyle.Alert)
                        
                        var dismissButton : UIAlertAction = UIAlertAction(title: "Okay", style: UIAlertActionStyle.Cancel)
                            {
                                alert in noAccountsAlert.dismissViewControllerAnimated(true, completion: nil)
                            }
                        noAccountsAlert.addAction(dismissButton)
                        
                        dispatch_async(dispatch_get_main_queue(), {self.presentViewController(noAccountsAlert, animated: true, completion: nil)})
    ...
    */
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        {
            // This was a method call??? had to change it!!!
